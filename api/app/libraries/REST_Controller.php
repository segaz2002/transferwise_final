...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function __construct()
	{
        ...
        $this->logHandle = fopen("requestsLog.log", "a+");
	}
    ...
    protected function computeRequestHash($requestHash,$m_id, $sysHashMac){
        $computed_hash = hash('sha512',$sysHashMac.$m_id);
        fwrite($this->logHandle, "SysHashMac ".$requestHash."<br/>");
        fwrite($this->logHandle, "SysHashMac ".$sysHashMac."<br/>");
        fwrite($this->logHandle, "Member ".$m_id." was computed to ". $computed_hash."<br>");

        if ($computed_hash == $requestHash){
            return TRUE;
        }
        else{
            $answer = array(
                'status' => $this->errorCode['badHashKeyComputation'],
                'description' => $this->errorDescription['badHashKeyComputation']
            );
            $this->response($answer);
        }
    }

    protected function  resolveArgs($params,$post){
        foreach ($params as $parameter){
            if(!(array_key_exists($parameter,$post))){
                $answer = array(
                    'status' => $this->errorCode['badRequest'],
                    'description' => $this->errorDescription['badRequest']
                );
                $this->response($answer);
            }
        }
    ...
    public function logRequest($action, $requestData){
        if(count($requestData) > 0 ){
            $logString = $requestData['m_id']."|".$action."|";
        }else{
            $logString = $requestData['m_id']."|".$action."|".$this->requestArgsToString($requestData);
        }
        fwrite($this->logHandle, $logString."<br/>");
        //write_file('./requestLog/request_log_'.mdate('%Y%m%d'), $requestString);
    }

    private function logResponse($responseData){
        //$responseString=
       // write_file('./responseLog/response_log_'.mdate('%Y%m%d'), $responseString);
    }

    static function requestArgsToString($args){
        //var_dump($args); exit;
        $argsString = '[';
        foreach ($args as $e => $v){
            if($e == 'reqHash' || $e == 'm_id' || $e == 'key'){continue;}
            $argsString.= $v.",";
        }
        return $argsString.']';
    }
